...
from src.core.yolo.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from src.core.yolo.utils.torch_utils import torch_distributed_zero_first
...
def _process_augmentations(self, img, labels, segments):
    img, labels, segments = copy_paste(img, labels, segments, probability=self.hyp['copy_paste'])
...
def _load_mosaic_common(self, index):
    """Common code for loading and processing a 4-mosaic of images"""
    labels4, segments4 = [], []
    s = self.img_size
    ...
    for i, index in enumerate(indices):
        ...
        # Initialize base image with first image if not done yet
        if img4 is None:
...
def load_mosaic(self, index):
    ...
    img4, labels4, segments4 = _load_mosaic_common(self, index)
    return _process_augmentations(self, img4, labels4, segments4)
...
def load_mosaic9(self, index):
    ...
    return _process_augmentations(self, img9, labels9, segments9)
...
def extract_boxes(path='../coco/'):  # from Src.flow_execute_paia.detection.utils.datasets import *; extract_boxes('../coco128')
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Usage: from Src.flow_execute_paia.detection.utils.datasets import *; autosplit('../coco')
    Arguments
